...
define(function(require,exports,module){
	...
	$.extend($.touchSlider.prototype,{
		...
		init : function(){
			...
			if(this.loop){  //复制首尾以便循环
				...
				if(this.isFullScreen){
					panels = panel.children("li");
					panels.css("width",100/panels.size() + "%");
				}
			}
			if(this.isFullScreen){
				panel.css('width',panels.size()*100+"%");
			}else{
			...
			if(this.isFullScreen){
				$(window).on("resize",function(){
					_this.steps = wrap.width();
					_this.slideTo(_this.curIndex);
					lastp.style.left = -_this.steps*(len+2) + "px";
				});
			}
		...
		setCoord : function(obj,x){
			this.useTransform && obj.css({"-webkit-transform":gv1 + x + 'px,0' + gv2,"transform":gv1 + x + 'px,0' + gv2}) || obj.css("left",x);
		},
