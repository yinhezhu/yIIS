...
define(function(require,exports,module){
	$.touchDate = function(container,options){
		...
		$.extend(this,{
			...
			middleDate : false,//是否是最大日期和最小日期之间
			minYear : 1900,//最小年份
			minMonth : 1,//最小月份
			minDay : 1,//最小日期
			maxYear : 2100,//最大年份
	...
	$.extend($.touchDate.prototype,{
		...
		init : function(){//初始化日期内容
			...
			var oDate = new Date();
			if(oDate.getTime() >= oMinDate.getTime() && oDate.getTime() <= oMaxDate.getTime()+86400000){
				this.isBetween = true;
			}
			if(this.isBetween){
			...
			if(this.isBetween){
				...
				if(this.monthList.children().eq(0).html() == 1){
					this.monthTop = (2 - this.month) * this.steps;
				}else{
					this.monthTop = (this.monthList.children().eq(0).html() + 1 - this.month)*this.steps;
				}
				this.monthList.css("top",this.monthTop + 'px');
				if(this.dayList.children().eq(0).html() == 1){
					this.dayTop = (2 - this.day) * this.steps;
				}else{
					this.dayTop = (this.dayList.children().eq(0).html() + 1 - this.day)*this.steps;
				}
