...
define(function(require,exports,module){
	$.touchDate = function(container,options){
		...
		this.container = $('dd' +container).eq(0);
		$.extend(this,{
			wrap : null,  //弹出框选择区域
	...
	$.extend($.touchDate.prototype,{
		...
		init : function(){
			...
			if(val){
				...
				alert(this.month);
				this.yearList.css("top",'-'+ (this.yearLength - 1)*this.steps +'px');
				this.yearTop = - (this.yearLength - 1)*this.steps;
				this.monthList.css("top",(2-this.month)*this.steps +'px');
		...
		enterIn : function(){
			...
			if(parseInt(this.month) <10){
				var myMonth = "0"+ this.month;
			}else{
				var myMonth = this.month;
			}
			if(parseInt(this.day)<10){
				var myDay = "0"+ this.day; 
			}else{
				var myDay = this.day;
			}
			this.container.html(this.year + "-" + myMonth + "-" + myDay);
			this.destroy();
		},
		destroy : function(){
			var that = this,
